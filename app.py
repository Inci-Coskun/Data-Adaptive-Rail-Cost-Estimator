...
# ── Aesthetic Light Theme CSS (Fully Dynamic) ──────────────────────────────────
st.markdown(f"""
<style>
...
def predict(country, city, length_km, tunnel_pct, num_stations,
    ...
    # FEATURE_ALL config'den çekiliyor, kolon sırası garanti altında
    X = pd.DataFrame([row])[FEATURE_ALL]
    return np.exp(model.predict(X)[0])

def band(pred): 
    # Değerler artık config'deki UNCERTAINTY_BANDS'den geliyor
...
with st.sidebar:
    ...
    # EĞER OTHER SEÇİLİRSE MANUEL GİRİŞ AÇILIR
...
if train_df is not None:
    ...
    # Skorları Yüzdeye Çevir (Maksimum teorik skor: 6.5)
    ...
    # Gösterilecek kolonları ve isimlerini ayarla
    display_cols = {
...
# ── Explainability: Why this price? (SHAP) ─────────────────────────────────────
st.divider()
...
try:
    ...
    # SHAP değerlerini alıyoruz
    sv = shap_values[0].values
    
    # SİHİRLİ MATEMATİK: Tüm etkiyi %100'e tamamlayacak şekilde oranlıyoruz
    ...
    fig_bar.update_layout(
        showlegend=False,
        xaxis_title="Relative Impact on Final Cost (%)", # Alt eksen ismi değişti
        yaxis_title="",
        margin=dict(t=50, b=50, l=10, r=60), # Sağda yüzdeler sığsın diye boşluk
        paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
        height=450,
        title=dict(text="Main Cost Drivers (Percentage Impact)", font=dict(size=14, color=UI_COLORS['text_main']), x=0.5)
    )
    
    # Izgara çizgilerini ve eksenleri estetik hale getir
    ...
    # Ortaya tek ve dev bir grafik olarak bas
